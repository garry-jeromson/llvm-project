...
private:
  ...
  /// Emit code to copy any register (including imaginary) to A.
  /// For A: no-op. For X: TXA. For Y: TYA. For imag: LDA_dp.
  void emitCopyToA(Block &MBB, BlockIt MBBI, Register SrcReg) {
    if (SrcReg == W65816::A)
      return;
    if (SrcReg == W65816::X) {
    ...
    if (W65816::IMAG16RegClass.contains(SrcReg)) {
      unsigned DPAddr = getImaginaryRegDPAddr(SrcReg);
      BuildMI(MBB, MBBI, MBBI->getDebugLoc(), TII->get(W65816::LDA_dp),
              W65816::A)
          .addImm(DPAddr);
      return;
    }
    // Fallback (shouldn't happen after RA)
    BuildMI(MBB, MBBI, MBBI->getDebugLoc(), TII->get(TargetOpcode::COPY),
            W65816::A)
        .addReg(SrcReg);
  }

  /// Emit code to copy A to any register (including imaginary).
  /// For A: no-op. For X: TAX. For Y: TAY. For imag: STA_dp.
  void emitCopyFromA(Block &MBB, BlockIt MBBI, Register DstReg) {
    if (DstReg == W65816::A)
      return;
    if (DstReg == W65816::X) {
    ...
    if (W65816::IMAG16RegClass.contains(DstReg)) {
      unsigned DPAddr = getImaginaryRegDPAddr(DstReg);
      BuildMI(MBB, MBBI, MBBI->getDebugLoc(), TII->get(W65816::STA_dp))
          .addReg(W65816::A)
          .addImm(DPAddr);
      return;
    }
  ...
  /// Emit code to copy any register (including imaginary) to Y.
  /// For Y: no-op. For X: TXY. For A: TAY. For imag: LDA_dp + TAY.
  void emitCopyToY(Block &MBB, BlockIt MBBI, Register SrcReg) {
    if (SrcReg == W65816::Y)
      return;
    if (SrcReg == W65816::X) {
  ...
  bool expandSCMP16rr(Block &MBB, BlockIt MBBI);
  bool expandSCMP16ri(Block &MBB, BlockIt MBBI);
  ...
  bool expandAND16ri(Block &MBB, BlockIt MBBI);
  bool expandOR16rr(Block &MBB, BlockIt MBBI);
  bool expandOR16ri(Block &MBB, BlockIt MBBI);
  bool expandXOR16rr(Block &MBB, BlockIt MBBI);
  bool expandXOR16ri(Block &MBB, BlockIt MBBI);
  ...
  bool expandSelect16Simple(Block &MBB, BlockIt MBBI, unsigned BranchOpcode);
...
bool W65816ExpandPseudo::expandMI(Block &MBB, BlockIt MBBI) {
  ...
  case W65816::SCMP16rr:
    return expandSCMP16rr(MBB, MBBI);
  case W65816::SCMP16ri:
    return expandSCMP16ri(MBB, MBBI);
  case W65816::BR_CC:
  ...
  case W65816::AND16ri:
    return expandAND16ri(MBB, MBBI);
  case W65816::OR16rr:
    return expandOR16rr(MBB, MBBI);
  case W65816::OR16ri:
    return expandOR16ri(MBB, MBBI);
  case W65816::XOR16rr:
    return expandXOR16rr(MBB, MBBI);
  case W65816::XOR16ri:
    return expandXOR16ri(MBB, MBBI);
  case W65816::SHL16rv:
  ...
  case W65816::Select16_EQ:
    return expandSelect16Simple(MBB, MBBI, W65816::BEQ);
  case W65816::Select16_NE:
    return expandSelect16Simple(MBB, MBBI, W65816::BNE);
  case W65816::Select16_CC:
    return expandSelect16Simple(MBB, MBBI, W65816::BCC);
  case W65816::Select16_CS:
    return expandSelect16Simple(MBB, MBBI, W65816::BCS);
...
bool W65816ExpandPseudo::expandADD16rr(Block &MBB, BlockIt MBBI) {
  ...
  // Check if A needs to be preserved across this expansion.
  // The expansion always goes through A, but A is not in Defs (to avoid
  // register pressure on the 3-register architecture). If A is live after
  // this instruction and DstReg != A, we must save/restore A.
  bool NeedToSaveA = false;
  if (DstReg != W65816::A) {
    ...
    NeedToSaveA = LiveRegs.contains(W65816::A);
  }
...
bool W65816ExpandPseudo::expandADD16ri(Block &MBB, BlockIt MBBI) {
  ...
  // Check if A needs to be preserved across this expansion.
  // The expansion goes through A, so if A is live and dst != A, save it.
  bool NeedToSaveA = false;
  if (DstReg != W65816::A && SrcReg != W65816::A) {
    LivePhysRegs LiveRegs(*TRI);
    LiveRegs.addLiveOuts(MBB);
    for (auto I = MBB.rbegin(); &*I != &MI; ++I) {
      LiveRegs.stepBackward(*I);
...
bool W65816ExpandPseudo::expandCMP16rr(Block &MBB, BlockIt MBBI) {
  ...
  // CMP16rr $src1, $src2 - UNSIGNED comparison
  // Uses non-destructive CMP/CPX/CPY hardware instructions.
  // Sets N, Z, C flags (NOT V). Does NOT clobber any register.
  //
  // Strategy for imaginary register src1:
  //   Use LDX_dp/LDY_dp to load to X or Y, then CPX/CPY.
  //   This avoids clobbering A. LDX/LDY do set N/Z flags but
  //   the CPX/CPY that follows overwrites those flags anyway.
  //   If X or Y needs saving, save to DP scratch (not stack, to avoid
  //   flag corruption from PLA). Restore via LDX_dp/LDY_dp BEFORE the
  //   CPX/CPY comparison — the comparison then sets the final flags.
  //
  // Wait — we can't restore the saved register before CPX/CPY because
  // we need the loaded value in the register for the comparison.
  // And we can't restore after, because LDX_dp/LDY_dp clobber N/Z flags.
  //
  // Better approach: if we must save a register, use PHP/PLP around the
  // restore. Or: save src2 to scratch, then we can use the index register
  // freely since src2 is in scratch.
  //
  // Actually, the simplest correct approach:
  //   1. Find a free index register (X or Y) that is NOT src2
  //   2. If none free, save one to scratch (we can restore after CPX/CPY
  //      because the branch instruction reads flags immediately after CMP,
  //      and the restore is AFTER CMP but BEFORE the branch in the same BB?
  //      No — we insert before MBBI which is the CMP pseudo, so everything
  //      goes before the next instruction which reads the flags.)
  //
  // Correct approach: After the CMP/CPX/CPY, the next instruction reads flags.
  // We must not insert anything between CPX/CPY and that next instruction
  // that clobbers flags. But we insert at MBBI position and erase MI,
  // so all our instructions go before the next real instruction. We need
  // PHP after CPX/CPY, restore register, PLP. But that's 4 extra instructions.
  //
  // Simplest correct approach for imaginary src1:
  //   - If X is free and X != src2: LDX_dp(src1), then compare with src2
  //   - If Y is free and Y != src2: LDY_dp(src1), then compare with src2
  //   - If both X and Y are occupied: use PHP/PLP to preserve flags around
  //     the register save/restore

  Register Src1Reg = MI.getOperand(0).getReg();
  Register Src2Reg = MI.getOperand(1).getReg();

  if (W65816::IMAG16RegClass.contains(Src1Reg)) {
    unsigned Src1DPAddr = getImaginaryRegDPAddr(Src1Reg);

    // Use LivePhysRegs to find a free index register
    LivePhysRegs LiveRegs(*TRI);
    LiveRegs.addLiveOuts(MBB);
    for (auto I = MBB.rbegin(); &*I != &MI; ++I)
      LiveRegs.stepBackward(*I);

    bool XLive = LiveRegs.contains(W65816::X);
    bool YLive = LiveRegs.contains(W65816::Y);
    bool ALive = LiveRegs.contains(W65816::A);

    // Pick the best register to load src1 into:
    // Prefer a free register that isn't src2
    Register UseReg = Register();
    bool NeedSave = false;
    bool UseAPath = false;

    if (!XLive && Src2Reg != W65816::X) {
      UseReg = W65816::X;
    } else if (!YLive && Src2Reg != W65816::Y) {
      UseReg = W65816::Y;
    } else if (!ALive) {
      // A is free — use LDA + CMP path (smaller than PHP/PLP fallback)
      UseAPath = true;
    } else if (Src2Reg != W65816::X) {
      // X is live but we must use it; save and restore
      UseReg = W65816::X;
      NeedSave = true;
    } else {
      // Y is live but we must use it; save and restore
      UseReg = W65816::Y;
      NeedSave = true;
    }

    if (UseAPath) {
      // A is dead — use LDA_dp(src1) + CMP for a compact comparison
      BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A)
          .addImm(Src1DPAddr);
      if (W65816::IMAG16RegClass.contains(Src2Reg)) {
        unsigned Src2DPAddr = getImaginaryRegDPAddr(Src2Reg);
      ...
      if (NeedSave) {
        // Save the register to DP scratch
        if (UseReg == W65816::X) {
          BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_dp))
              .addReg(W65816::X)
              .addImm(SCRATCH_DP_ADDR);
      ...
      // Load imaginary src1 to the chosen register
      if (UseReg == W65816::X) {
        BuildMI(MBB, MBBI, DL, TII->get(W65816::LDX_dp), W65816::X)
            .addImm(Src1DPAddr);
      } else {
        BuildMI(MBB, MBBI, DL, TII->get(W65816::LDY_dp), W65816::Y)
            .addImm(Src1DPAddr);
      }

      // Now compare UseReg with src2
      if (W65816::IMAG16RegClass.contains(Src2Reg)) {
        unsigned Src2DPAddr = getImaginaryRegDPAddr(Src2Reg);
        if (UseReg == W65816::X)
          BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_dp)).addImm(Src2DPAddr);
        else
          BuildMI(MBB, MBBI, DL, TII->get(W65816::CPY_dp)).addImm(Src2DPAddr);
      } else if (Src2Reg == W65816::A) {
        // Need to store A to a temp location for CPX/CPY
        // We can't use SCRATCH_DP_ADDR if we saved a register there
        if (NeedSave) {
          // Use SCRATCH_DP_ADDR - 2 as a second scratch location
          BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_dp))
              .addReg(W65816::A)
              .addImm(SCRATCH_DP_ADDR - 2);
          if (UseReg == W65816::X)
            BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_dp))
                .addImm(SCRATCH_DP_ADDR - 2);
          else
            BuildMI(MBB, MBBI, DL, TII->get(W65816::CPY_dp))
                .addImm(SCRATCH_DP_ADDR - 2);
        } else {
          BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_dp))
              .addReg(W65816::A)
        ...
        // src1 loaded to Y (since UseReg != src2), compare Y with X
        BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_dp))
            .addReg(W65816::X)
            .addImm(NeedSave ? SCRATCH_DP_ADDR - 2 : SCRATCH_DP_ADDR);
        BuildMI(MBB, MBBI, DL, TII->get(W65816::CPY_dp))
            .addImm(NeedSave ? SCRATCH_DP_ADDR - 2 : SCRATCH_DP_ADDR);
      } else if (Src2Reg == W65816::Y) {
        // src1 loaded to X (since UseReg != src2), compare X with Y
        BuildMI(MBB, MBBI, DL, TII->get(W65816::STY_dp))
            .addReg(W65816::Y)
            .addImm(NeedSave ? SCRATCH_DP_ADDR - 2 : SCRATCH_DP_ADDR);
        BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_dp))
      ...
      if (NeedSave) {
        // Restore the saved register. We use PHP/PLP to preserve flags.
  ...
  } else if (Src1Reg == W65816::A) {
    // src1 is A - use CMP directly
    if (Src2Reg == W65816::A) {
      // CMP A with A - always equal. Use CMP with DP scratch.
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_dp))
          .addReg(W65816::A)
          .addImm(SCRATCH_DP_ADDR);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CMP_dp))
  ...
  } else if (Src1Reg == W65816::X) {
    // src1 is X - use CPX
    if (Src2Reg == W65816::A) {
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_dp))
          .addReg(W65816::A)
          .addImm(SCRATCH_DP_ADDR);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_dp)).addImm(SCRATCH_DP_ADDR);
    } else if (Src2Reg == W65816::X) {
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_dp))
          .addReg(W65816::X)
          .addImm(SCRATCH_DP_ADDR);
      ...
      unsigned DPAddr = getImaginaryRegDPAddr(Src2Reg);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_dp)).addImm(DPAddr);
    }
  } else if (Src1Reg == W65816::Y) {
    // src1 is Y - use CPY
    if (Src2Reg == W65816::A) {
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_dp))
          .addReg(W65816::A)
          .addImm(SCRATCH_DP_ADDR);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CPY_dp)).addImm(SCRATCH_DP_ADDR);
    } else if (Src2Reg == W65816::X) {
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_dp))
          .addReg(W65816::X)
          .addImm(SCRATCH_DP_ADDR);
      ...
      unsigned DPAddr = getImaginaryRegDPAddr(Src2Reg);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CPY_dp)).addImm(DPAddr);
    }
  }

  // Flags are now set based on src1 - src2:
  // - N = 1 if result is negative (in 16-bit context)
  // - Z = 1 if result is zero (src1 == src2)
  // - C = 1 if no borrow (src1 >= src2 unsigned)
  // - V is NOT set by CMP/CPX/CPY (use SCMP16rr for signed comparisons)
...
bool W65816ExpandPseudo::expandCMP16ri(Block &MBB, BlockIt MBBI) {
  ...
  if (SrcReg == W65816::A) {
    ...
    // Load imaginary register to X or Y, then use CPX/CPY with immediate.
    // This avoids clobbering A (Defs=[P] only).
    unsigned DPAddr = getImaginaryRegDPAddr(SrcReg);
    ...
    if (!XLive) {
      ...
      // A is dead — use LDA_dp + CMP_imm16 (5 bytes, no save/restore)
      BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A)
          .addImm(DPAddr);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CMP_imm16))
          .addReg(W65816::A)
      ...
      // All registers live. Save X to scratch, use it, restore with
      // PHP/PLP to preserve flags.
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_dp))
          .addReg(W65816::X)
          .addImm(SCRATCH_DP_ADDR);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::LDX_dp), W65816::X)
          .addImm(DPAddr);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::CPX_imm16)).addImm(Imm);
      // Restore X, preserving flags from CPX
      buildMI(MBB, MBBI, W65816::PHP);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::LDX_dp), W65816::X)
          .addImm(SCRATCH_DP_ADDR);
      buildMI(MBB, MBBI, W65816::PLP);
...
bool W65816ExpandPseudo::expandSCMP16rr(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // SCMP16rr $src1, $src2 - SIGNED comparison
  // Uses SEC + SBC to correctly set N, Z, C, AND V flags.
  // Clobbers A (declared in Defs).
...
bool W65816ExpandPseudo::expandSCMP16ri(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // SCMP16ri $src, $imm - SIGNED comparison with immediate
  // Uses SEC + SBC to correctly set N, Z, C, AND V flags.
  // Clobbers A (declared in Defs).
  ...
  // SEC + SBC to correctly set V flag for signed comparison
  buildMI(MBB, MBBI, W65816::SEC);
  BuildMI(MBB, MBBI, DL, TII->get(W65816::SBC_imm16), W65816::A).addImm(Imm);
...
bool W65816ExpandPseudo::expandSHL16ri(Block &MBB, BlockIt MBBI) {
  ...
  // Output is GPR16 - result moved from A to destination if needed
  ...
  // Move result from A to destination if needed
  if (DstReg == W65816::X) {
    buildMI(MBB, MBBI, W65816::TAX);
  } else if (DstReg == W65816::Y) {
    buildMI(MBB, MBBI, W65816::TAY);
...
bool W65816ExpandPseudo::expandAND16ri(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // AND16ri $dst, $src, $imm
  // Expand to: AND #imm (with transfers as needed)
...
bool W65816ExpandPseudo::expandOR16ri(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // OR16ri $dst, $src, $imm
  // Expand to: ORA #imm (with transfers as needed)
  ...
  // ORA immediate
  BuildMI(MBB, MBBI, DL, TII->get(W65816::ORA_imm16), W65816::A)
      .addReg(W65816::A)
      .addImm(Imm);
...
bool W65816ExpandPseudo::expandXOR16ri(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // XOR16ri $dst, $src, $imm
  // Expand to: EOR #imm (with transfers as needed)
...
bool W65816ExpandPseudo::expandSHL16rv(Block &MBB, BlockIt MBBI) {
  ...
  } else if (W65816::IMAG16RegClass.contains(AmtReg)) {
    // Load imaginary register to X via A
    buildMI(MBB, MBBI, W65816::PHA); // save A (source)
    unsigned DPAddr = getImaginaryRegDPAddr(AmtReg);
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A).addImm(DPAddr);
    buildMI(MBB, MBBI, W65816::TAX);
    buildMI(MBB, MBBI, W65816::PLA); // restore A (source)
  }
  ...
  // Move result from A to destination and restore A at the start of DoneBB.
  // We insert in reverse order since each insert goes to DoneBB->begin().
  // Final order: [dst-move] [PLA if needed] [rest of block]

  if (NeedToSaveA) {
    BuildMI(*DoneBB, DoneBB->begin(), DL, TII->get(W65816::PLA));
  }
  ...
  if (DstReg == W65816::X) {
    ...
    unsigned DPAddr = getImaginaryRegDPAddr(DstReg);
    BuildMI(*DoneBB, DoneBB->begin(), DL, TII->get(W65816::STA_dp))
        .addReg(W65816::A)
        .addImm(DPAddr);
  }
  // If DstReg == A, result is already there
...
bool W65816ExpandPseudo::expandSRL16rv(Block &MBB, BlockIt MBBI) {
  ...
  // Move result from A to destination and restore A at the start of DoneBB.
  // Insert in reverse order since each insert goes to DoneBB->begin().
  if (NeedToSaveA) {
    BuildMI(*DoneBB, DoneBB->begin(), DL, TII->get(W65816::PLA));
  }
...
bool W65816ExpandPseudo::expandLDAindirect(Block &MBB, BlockIt MBBI) {
  ...
  if (W65816::IMAG16RegClass.contains(PtrReg)) {
    // Imaginary register - load from Direct Page to A
    unsigned DPAddr = getImaginaryRegDPAddr(PtrReg);
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A).addImm(DPAddr);
...
bool W65816ExpandPseudo::expandSTAindirect(Block &MBB, BlockIt MBBI) {
  ...
  // Special case: ValReg is imaginary and PtrReg is A.
  // Loading val from DP to A would clobber the pointer in A.
  // Fix: store ptr to stack slot FIRST, then push val.
  if (ValIsImag && PtrReg == W65816::A) {
    // Step 1: Store ptr (in A) to stack slot at original offset
    BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_sr))
        .addReg(W65816::A)
        .addImm(StackSlotOffset);

    // Step 2: Load val from DP, push to hardware stack
    unsigned DPAddr = getImaginaryRegDPAddr(ValReg);
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A).addImm(DPAddr);
    buildMI(MBB, MBBI, W65816::PHA);
    ...
    // Step 4: Restore val to A from hardware stack (SP restored)
    buildMI(MBB, MBBI, W65816::PLA);
  ...
  // General case: push val first, then store ptr.
  // Step 1: Save val to hardware stack
  // After this, SP decreases by 2, so stack slot is at offset+2
  if (ValIsImag) {
    // Imaginary register - load from Direct Page to A, then push
    unsigned DPAddr = getImaginaryRegDPAddr(ValReg);
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A).addImm(DPAddr);
    buildMI(MBB, MBBI, W65816::PHA);
...
bool W65816ExpandPseudo::expandSPILL_GPR16(Block &MBB, BlockIt MBBI) {
  ...
  if (SrcReg == W65816::A) {
    ...
    if (SrcReg == W65816::X) {
      ...
      // Load imaginary register's DP address into A.
      unsigned DPAddr = getImaginaryRegDPAddr(SrcReg);
      BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_dp), W65816::A)
          .addImm(DPAddr);
    }
...
bool W65816ExpandPseudo::expandLOAD_GPR16_abs(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // LOAD_GPR16_abs $dst, $addr
  // Load from absolute address to any GPR16 register.
  // For A: LDA_abs
  // For X: LDX_abs
  // For Y: LDY_abs
  // For imaginary: LDA_abs + STA_dp (with A save/restore if live)

  Register DstReg = MI.getOperand(0).getReg();
  MachineOperand &AddrOp = MI.getOperand(1);

  static const unsigned LOAD_ABS_SCRATCH_DP = 0xFC;

  if (DstReg == W65816::A) {
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDA_abs), W65816::A).add(AddrOp);
  } else if (DstReg == W65816::X) {
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDX_abs), W65816::X).add(AddrOp);
  } else if (DstReg == W65816::Y) {
    BuildMI(MBB, MBBI, DL, TII->get(W65816::LDY_abs), W65816::Y).add(AddrOp);
  } else if (W65816::IMAG16RegClass.contains(DstReg)) {
    // Load to imaginary register: LDA_abs + STA_dp
    LivePhysRegs LiveRegs(*TRI);
    LiveRegs.addLiveOuts(MBB);
    for (auto I = MBB.rbegin(); &*I != &MI; ++I) {
      LiveRegs.stepBackward(*I);
...
bool W65816ExpandPseudo::expandSTORE_GPR16_abs(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // STORE_GPR16_abs $src, $addr
  // Store from any GPR16 register to absolute address.
  // For A: STA_abs
  // For X: STX_abs
  // For Y: STY_abs
  // For imaginary: LDA_dp + STA_abs (with A save/restore if live)

  Register SrcReg = MI.getOperand(0).getReg();
  MachineOperand &AddrOp = MI.getOperand(1);

  static const unsigned STORE_ABS_SCRATCH_DP = 0xFC;
  ...
  if (SrcReg == W65816::A) {
    ...
    BuildMI(MBB, MBBI, DL, TII->get(W65816::STX_abs))
        .addReg(W65816::X)
        .add(AddrOp);
  } else if (SrcReg == W65816::Y) {
    BuildMI(MBB, MBBI, DL, TII->get(W65816::STY_abs))
        .addReg(W65816::Y)
        .add(AddrOp);
  } else if (W65816::IMAG16RegClass.contains(SrcReg)) {
    // Store from imaginary register: LDA_dp + STA_abs
    LivePhysRegs LiveRegs(*TRI);
    LiveRegs.addLiveOuts(MBB);
    for (auto I = MBB.rbegin(); &*I != &MI; ++I) {
      LiveRegs.stepBackward(*I);
...
bool W65816ExpandPseudo::expandZEXT8_GPR16(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // ZEXT8_GPR16 $dst, $src
  // Zero-extend 8-bit value to 16-bit using AND #$00FF.
  // No tied operand between src and dst, so the register allocator can
  // place them in different physical registers, avoiding ACC16 pressure.
  //
  // Defs=[A, P] on the pseudo tells the RA that A is always clobbered,
  // so no manual A save/restore is needed.
  //
  // Expansion:
  //   1. Get src into A (TXA/TYA/LDA_dp if needed)
  //   2. AND #$00FF
  //   3. Move result to dst (TAX/TAY/STA_dp if needed)

  Register DstReg = MI.getOperand(0).getReg();
  Register SrcReg = MI.getOperand(1).getReg();

  // Step 1: Get source value into A.
  if (SrcReg == W65816::A) {
    // Already in A.
  } else if (SrcReg == W65816::X) {
    buildMI(MBB, MBBI, W65816::TXA);
  ...
  // Step 2: AND with 0x00FF to zero-extend.
  BuildMI(MBB, MBBI, DL, TII->get(W65816::AND_imm16), W65816::A)
      .addReg(W65816::A)
      .addImm(0x00FF);

  // Step 3: Move result from A to destination.
  if (DstReg == W65816::A) {
    // Already in A.
  } else if (DstReg == W65816::X) {
    buildMI(MBB, MBBI, W65816::TAX);
...
bool W65816ExpandPseudo::expandSEXT8_GPR16(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // SEXT8_GPR16 $dst, $src
  // Sign-extend 8-bit value to 16-bit using branchless arithmetic:
  //   result = (val & 0xFF) ^ 0x80 - 0x80
  //
  // This works because:
  //   - Positive (0x00-0x7F): XOR flips bit7 → 0x80-0xFF, minus 0x80 →
  //   0x00-0x7F
  //   - Negative (0x80-0xFF): XOR flips bit7 → 0x00-0x7F, minus 0x80 →
  //   0xFF80-0xFFFF
  //
  // Defs=[A, P] on the pseudo tells the RA that A is always clobbered.
  //
  // Expansion:
  //   1. Get src into A
  //   2. AND #$00FF (clear high byte)
  //   3. EOR #$0080 (flip sign bit)
  //   4. SEC; SBC #$0080 (subtract 0x80, completing sign extension)
  ...
  // Step 2: AND with 0x00FF to isolate the low byte.
  BuildMI(MBB, MBBI, DL, TII->get(W65816::AND_imm16), W65816::A)
      .addReg(W65816::A)
      .addImm(0x00FF);

  // Step 3: EOR with 0x0080 to flip the sign bit.
  BuildMI(MBB, MBBI, DL, TII->get(W65816::EOR_imm16), W65816::A)
      .addReg(W65816::A)
      .addImm(0x0080);

  // Step 4: SEC + SBC #$0080 to subtract 128 (completes sign extension).
  buildMI(MBB, MBBI, W65816::SEC);
  BuildMI(MBB, MBBI, DL, TII->get(W65816::SBC_imm16), W65816::A).addImm(0x0080);

  // Step 5: Move result from A to destination.
  if (DstReg == W65816::A) {
    // Already in A.
  } else if (DstReg == W65816::X) {
    buildMI(MBB, MBBI, W65816::TAX);
...
bool W65816ExpandPseudo::expandLOAD8_ZEXT_GPR16_abs(Block &MBB, BlockIt MBBI) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();

  // LOAD8_ZEXT_GPR16_abs $dst, $addr
  // Load 8-bit value from absolute address, zero-extend to 16-bit.
  // Combined load+zext avoids the ACC16 register pressure issue.
  //
  // Defs=[A, P] on the pseudo tells the RA that A is always clobbered.
  //
  // Expansion:
...
bool W65816ExpandPseudo::expandMOV16ri(Block &MBB, BlockIt MBBI) {
  ...
  if (DstReg == W65816::A) {
    ...
    // Must save/restore A if it's live, since this expansion clobbers A
    // but MOV16ri's Defs don't include A.
    unsigned DPAddr = getImaginaryRegDPAddr(DstReg);

    static const unsigned MOV_SCRATCH_DP = 0xFC;
    LivePhysRegs LiveRegs(*TRI);
    LiveRegs.addLiveOuts(MBB);
    for (auto I = MBB.rbegin(); &*I != &MI; ++I)
      LiveRegs.stepBackward(*I);
...
bool W65816ExpandPseudo::expandSelect16Simple(Block &MBB, BlockIt MBBI,
                                              unsigned BranchOpcode) {
  MachineInstr &MI = *MBBI;
  DebugLoc DL = MI.getDebugLoc();
  MachineFunction *MF = MBB.getParent();

  // Select16_xx $dst, $trueVal, $falseVal
  // The comparison has already been done (P flags are set).
  // Select trueVal if the condition is true, else falseVal.
  // Uses a single branch instruction (BEQ/BNE/BCC/BCS).
  ...
  // Branch on condition to TrueMBB, fall through to FalseMBB.
  BuildMI(MBB, MBBI, DL, TII->get(BranchOpcode)).addMBB(TrueMBB);
  BuildMI(MBB, MBBI, DL, TII->get(W65816::BRA)).addMBB(FalseMBB);
  ...
  // FalseMBB: copy falseVal to dst (falls through to SinkMBB).
  if (FalseReg != DstReg) {
    if (FalseReg == W65816::X) {
      BuildMI(FalseMBB, DL, TII->get(W65816::TXA));
    } else if (FalseReg == W65816::Y) {
...
bool W65816ExpandPseudo::expandLDA8indirect(Block &MBB, BlockIt MBBI) {
  ...
  // Move pointer to A if not already there (handles imaginary regs)
  emitCopyToA(MBB, MBBI, PtrReg);
...
bool W65816ExpandPseudo::expandSTA8indirect(Block &MBB, BlockIt MBBI) {
  ...
  // Step 2: Get pointer into A (handles A/X/Y/imaginary)
  emitCopyToA(MBB, MBBI, PtrReg);
  ...
  if (SrcInA) {
    ...
    // Handles X/Y/imaginary registers
    emitCopyToA(MBB, MBBI, SrcReg);
  }
...
bool W65816ExpandPseudo::expandLDA8indirectIdx(Block &MBB, BlockIt MBBI) {
  ...
  // First, get the pointer into A and store to stack (handles imaginary)
  emitCopyToA(MBB, MBBI, PtrReg);
  ...
  // Get index into Y (handles imaginary)
  emitCopyToY(MBB, MBBI, IdxReg);
...
bool W65816ExpandPseudo::expandSTA8indirectIdx(Block &MBB, BlockIt MBBI) {
  ...
  bool SrcIsImag = W65816::IMAG16RegClass.contains(SrcReg);
  ...
  // Get pointer into A and store to stack (handles imaginary)
  emitCopyToA(MBB, MBBI, PtrReg);
  ...
  auto StorePtr2 =
      BuildMI(MBB, MBBI, DL, TII->get(W65816::STA_sr)).addReg(W65816::A);
  if (SlotOp.isFI()) {
    StorePtr2.addFrameIndex(SlotOp.getIndex());
  } else {
    StorePtr2.addImm(SlotOp.getImm());
  }
...
bool W65816ExpandPseudo::expandLDAindexedDPY(Block &MBB, BlockIt MBBI) {
  ...
  // Move result to destination register if needed (handles imaginary)
  emitCopyFromA(MBB, MBBI, DstReg);
...
bool W65816ExpandPseudo::expandSTAindexedDPY(Block &MBB, BlockIt MBBI) {
  ...
  if (IdxInY) {
    ...
    // Index in a GPR (not A, X, or Y) - handles imaginary
    emitCopyToY(MBB, MBBI, IdxReg);
    if (!ValInA) {
      if (ValInX) {
        buildMI(MBB, MBBI, W65816::TXA);
...
bool W65816ExpandPseudo::expandCOPY_IMAG_TO_IMAG(Block &MBB, BlockIt MBBI) {
  ...
  // Copy between two imaginary registers via LDA/STA.
  // Must save/restore A since the register allocator does not expect
  // COPY operations to clobber registers other than the destination.
  Register DstReg = MI.getOperand(0).getReg();
  Register SrcReg = MI.getOperand(1).getReg();
  unsigned SrcDPAddr = getImaginaryRegDPAddr(SrcReg);
  unsigned DstDPAddr = getImaginaryRegDPAddr(DstReg);

  // Check if P (processor flags) is live after this instruction.
  // If so, we need to preserve flags since PLA clobbers N/Z.
  LivePhysRegs LiveRegs(*TRI);
  LiveRegs.addLiveOuts(MBB);
  for (auto I = MBB.rbegin(); &*I != &MI; ++I)
    LiveRegs.stepBackward(*I);
  bool PLive = LiveRegs.contains(W65816::P);

  if (PLive) {
    // P is live - preserve flags with PHP/PLP
    buildMI(MBB, MBBI, W65816::PHP);
  }

  // Save A (the copy goes through A but must not clobber it)
  buildMI(MBB, MBBI, W65816::PHA);
